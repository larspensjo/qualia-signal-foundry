...
`Agents.md`, `docs/ProjectFrame/ProjectWorkflow.md`, `docs/DecisionLog.md`, the
architecture documents and code areas the brief touches, and neighboring
subsystems the brief does *not* mention — absence from the brief is where
blindspots live.
...
- **Unstated assumptions** the brief treats as settled without evidence —
  especially ones contradicted by the code, a DecisionLog entry, or an
  experiment result.
...
- **Missing interactions**: subsystems, documents, or in-flight work the change
  will touch that the brief doesn't name.
