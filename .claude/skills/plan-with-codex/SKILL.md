...
an existing `Idea.*.md` or an already fleshed-out description, don't
re-interrogate — read it, confirm your understanding, and probe only the gaps.
...
- Constraints (architecture rules, safety boundaries, relevant DecisionLog
  entries you know of).
- Files, modules, and documents the work touches.
- Open questions that remain deliberately open.
- Whether the work probes a simulation mechanism (experiment territory) or is
  routine engineering — per `docs/ProjectFrame/ProjectWorkflow.md`.
...
planning conventions (Agents.md, docs/ProjectFrame/ProjectWorkflow.md) and the
code it touches.
...
verifiable?), risks the plan ignores, and conflicts with repo conventions or
docs/DecisionLog.md.
