...
| `Experiment.RealtimeVolitionReadOnlyInspection` | High | Planned | Can the realtime model call `inspect_volition_state` and `select_volition_goals` tools to explain active goals and arbitration results in a live voice session? |
| `Experiment.RealtimeVolitionContextInjection` | High | Planned | Does injecting a compact, trace-backed volition context packet before `response.create` subtly steer responses toward active goals without displacing user intent? |
