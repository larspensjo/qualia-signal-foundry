use crate::state::{SessionConfig, SessionLimit, SessionState};
...
mod tests {
    use super::*;
    use crate::state::{MemorySourceConfig, SessionEndReason};
    ...
    fn previous_with(end_reason: Option<SessionEndReason>, turns: usize) -> SessionState {
        ...
        for index in 0..turns {
            state.turns.push(crate::state::tests::fake_turn(index));
        }
    ...
    fn live_state_clears_volatile_audio_on_resume() {
        ...
        prev.live.active_response = Some(crate::live_state::ActiveResponseState {
            response_id: Some("response-1".to_string()),
            partial_text: "hello".to_string(),
            status: crate::live_state::ResponseStatus::Streaming,
            observed_at: std::time::SystemTime::UNIX_EPOCH,
            audio_marker: Some("marker".to_string()),
        });
        prev.live.partial_transcript = Some(crate::live_state::PartialTranscript {
            exchange_index: 0,
        ...
        assert_eq!(
            cont.live.runtime_phase,
            crate::live_state::RuntimePhase::Idle
        );
    }
